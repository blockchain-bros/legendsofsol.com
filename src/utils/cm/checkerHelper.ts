...
export const allocationChecker = async (
  ...
  try {
    ...
    if (mintCounter) {
      ...
      console.log("Allocation Guard not Initialized!");
...
export const getSolanaTime = async (umi: Umi) => {
  ...
  if (!solanaTime) solanaTime = BigInt(Date.now());
  return solanaTime;
};
...
export const checkSolBalanceRequired = (
  ...
  guards.forEach((guard) => {
    const checkGuard = guard.guards.freezeSolPayment || guard.guards.solPayment;    
    if (checkGuard.__option === "Some") {
