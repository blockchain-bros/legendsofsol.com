...
export class Swiper implements SwiperProps {
  ...
  constructor({ element, id, onDismiss }: SwiperProps) {
    this.id = id;
  ...
  private showRibbons = () => {
    const ribbonLike = this.element.querySelector(".ribbon-like");
    const ribbonDislike = this.element.querySelector(".ribbon-dislike");
  ...
  private dismiss = (
    direction: number,
    swipeOperation: SwipeOperation = SwipeOperation.SWIPE
  ) => {
    this.startPoint = null;
    ...
    this.element.style.transform = `translate(${
      direction * window.innerWidth * 2
    }px, ${this.offsetY}px) rotate(${60 * direction}deg)`;
    this.element.classList.add("dismissing");
    setTimeout(() => this.element.remove(), 300);

    if (typeof this.onDismiss === "function") {
      const swipeDirection =
        direction === 1 ? SwipeAction.LIKE : SwipeAction.DISLIKE;
      this.onDismiss(this.element, this.id, swipeDirection, swipeOperation);
    }
  };
